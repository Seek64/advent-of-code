...
def find_combination(tgt: int, intermediate_result: int, op: list[int]) -> bool:
    ...
    n, *op = op
    return (find_combination(tgt, intermediate_result + n, op)
            or find_combination(tgt, intermediate_result * n, op)
            or find_combination(tgt, int(str(intermediate_result) + str(n)), op))
