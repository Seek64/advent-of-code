...
def fix_ordering(pages: list[int], rules: dict[int: int]) -> list[int]:
    for i, page in enumerate(pages):
        if page in rules:
            for page_rule in rules[page]:
                if page_rule in pages[:i]:
                    j = pages.index(page_rule)
...
for page_matches in re.findall(r"(\d+(,\d+)+)", input_str):
    ...
    while not ordering_correct(page_list, ordering_rules):
        fix_ordering(page_list, ordering_rules)
